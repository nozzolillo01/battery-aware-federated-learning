...
class BaseStrategy(FedAvg):
    ...
    def _prepare_evaluation_metrics(self, server_round: int, loss: float, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Detect if these metrics are server-side or client-side
        is_server_eval = "test_accuracy_server" in metrics

        results = {
            "round": server_round,
            # Do not log a generic 'loss' key to W&B; use explicit ones instead
        ...
        # Add explicit metric names requested
        if is_server_eval:
            results["test_accuracy_server"] = round(float(metrics.get("test_accuracy_server", 0)), 4)
            results["test_loss_server"] = round(loss, 4)
        else:
            results["val_accuracy_client"] = round(float(metrics.get("accuracy", 0)), 4)
            results["val_loss_client"] = round(loss, 4)

        # Attach last aggregated training metrics with requested names
        ...
        # Note: gap_train_val_client intentionally removed as per request
    ...
    def _log_results_to_wandb(self, server_round: int, analysis_results: Dict[str, Any]) -> None:
        ...
        try:
            # Append current values to series (None when missing)
            ...
            # Do not fail the run if chart creation has issues
            pass
    ...
    def evaluate(
        ...
        # Store last server-side eval to combine with client-side eval later.
